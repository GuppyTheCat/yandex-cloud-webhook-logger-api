...
- [Webhook Logger API](#webhook-logger-api)
  - [📑 Оглавление](#-оглавление)
  - [📋 Описание задачи](#-описание-задачи)
  - [⚡ Оптимизация производительности (Latency \< 200ms)](#-оптимизация-производительности-latency--200ms)
  - [🏗 Архитектура](#-архитектура)
  - [🛠 Компоненты системы](#-компоненты-системы)
  - [🚀 Установка и Деплой](#-установка-и-деплой)
    - [Предварительные требования](#предварительные-требования)
    - [Шаг 1: Настройка окружения и прав доступа](#шаг-1-настройка-окружения-и-прав-доступа)
    - [Шаг 2: База данных YDB](#шаг-2-база-данных-ydb)
    - [Шаг 3: Секреты Lockbox](#шаг-3-секреты-lockbox)
    - [Шаг 4: Очередь YMQ](#шаг-4-очередь-ymq)
    - [Шаг 5: Деплой webhook-receiver](#шаг-5-деплой-webhook-receiver)
    - [Шаг 6: Деплой webhook-processor](#шаг-6-деплой-webhook-processor)
    - [Шаг 7: Деплой logs-api](#шаг-7-деплой-logs-api)
  - [🧪 Тестирование](#-тестирование)
    - [Использование скрипта (Рекомендуется)](#использование-скрипта-рекомендуется)
    - [Примеры Curl команд](#примеры-curl-команд)
...
*   Установленный [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) (для работы с YMQ)
    ...
    sudo ./aws/install
    ```
*   *(Опционально)* [jq](https://jqlang.org) — для форматирования JSON вывода в логах и тестах.
...
Если вы хотите протестировать API вручную, используйте следующие команды.

**1. Генерация подписи HMAC-SHA256:**

```bash
export SECRET_KEY="your-secret-key"
export PAYLOAD='{"event_type":"payment.success","data":{"amount":1000}}'

# Расчет HMAC-SHA256 с помощью openssl
SIGNATURE="sha256=$(echo -n "$PAYLOAD" | openssl dgst -sha256 -hmac "$SECRET_KEY" | awk '{print $2}')"
```

**2. Отправка Webhook события:**
...
**3. Получение истории логов:**

```bash
# Получить последние 10 событий
curl "$LOGS_API_URL?limit=10" | jq .

# Фильтрация по типу события
curl "$LOGS_API_URL?limit=10&event_type=payment.success" | jq .
```
